        ...
        for model_no in range(len(self.base_classifiers)):
            ...
            for j in range(0, n_folds):
                ...
                if self.evaluation is not None:
                    ...
                    loss_avg += loss
                # Bit of a hack as some pass back only a single y value others pass back both
                if predicted_y_proba.shape[1] == 1:
                    self.stacking_train.ix[self.stacking_train.index.isin(idx1pd), model_no] = predicted_y_proba.ravel()
                else:
                    self.stacking_train.ix[self.stacking_train.index.isin(idx1pd), model_no] = predicted_y_proba[:, 1]
